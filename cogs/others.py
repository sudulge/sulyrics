...
class Others(commands.Cog):
    ...
    @slash_command(guild_ids=[723892698435551324], description="check bot's response latency")
    async def ping(self, ctx):
        embed = discord.Embed(title="Pong!", description=f"Delay: {self.bot.latency} seconds", color=0xf5a9a9)
        await ctx.respond(embed=embed)


    @slash_command(name="정보", guild_ids=[723892698435551324], description="수리릭 정보")
    ...
    @slash_command(name="자기소개", guild_ids=[723892698435551324], description="사용자 정보")
    ...
    @slash_command(name="삭제", guild_ids=[723892698435551324], description="메시지 삭제")
    ...
    @slash_command(name="고정", guild_ids=[723892698435551324], description="메시지 고정")
    async def pinMessage(self, ctx, message_id: Option(str, "메시지 아이디 입력")):
        message = await ctx.fetch_message(int(message_id))
        await message.pin()


    @slash_command(name="구글", guild_ids=[723892698435551324], description="구글 검색")
    ...
    @slash_command(name="오늘의라인", guild_ids=[723892698435551324], description="롤 라인 뽑기")
    async def lolLane(self, ctx):
        lane = ["탑","미드","원딜","서폿","정글"]
        await ctx.respond(f"오늘의 라인은`{lane[random.randint(0, 4)]}`", ephemeral=True)


    @slash_command(name="로또", guild_ids=[723892698435551324], description="로또")
